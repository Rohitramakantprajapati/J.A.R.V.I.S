...
def _missing_action_factory(module_name: str, symbol_name: str, error: Exception):
    def _missing_action(parameters=None, response=None, player=None):
        msg = (
            f"Action unavailable: {symbol_name} ({module_name}). "
            f"Dependency/import error: {error}"
        )
...
def _safe_import_action(module_name: str, symbol_name: str):
    try:
        module = importlib.import_module(module_name)
        return getattr(module, symbol_name)
    except Exception as exc:
        return _missing_action_factory(module_name, symbol_name, exc)


flight_finder = _safe_import_action("actions.flight_finder", "flight_finder")
open_app = _safe_import_action("actions.open_app", "open_app")
weather_action = _safe_import_action("actions.weather_report", "weather_action")
send_message = _safe_import_action("actions.send_message", "send_message")
reminder = _safe_import_action("actions.reminder", "reminder")
computer_settings = _safe_import_action("actions.computer_settings", "computer_settings")
screen_process = _safe_import_action("actions.screen_processor", "screen_process")
youtube_video = _safe_import_action("actions.youtube_video", "youtube_video")
cmd_control = _safe_import_action("actions.cmd_control", "cmd_control")
desktop_control = _safe_import_action("actions.desktop", "desktop_control")
browser_control = _safe_import_action("actions.browser_control", "browser_control")
file_controller = _safe_import_action("actions.file_controller", "file_controller")
code_helper = _safe_import_action("actions.code_helper", "code_helper")
dev_agent = _safe_import_action("actions.dev_agent", "dev_agent")
web_search_action = _safe_import_action("actions.web_search", "web_search")
computer_control = _safe_import_action("actions.computer_control", "computer_control")
...
class JarvisLive:
    ...
    async def _send_startup_greeting(self):
        if self._startup_greeted or not self.session:
            return
        ...
        await self.session.send_client_content(
            turns={
                "parts": [{
                    "text": (
                        "System boot complete. Greet the user now in one short, friendly sentence. "
...
def main():
    try:
        ...
        print("[JARVIS] ❌ Failed to import UI module (ui.py).")
        traceback.print_exc()
        return
    ...
    try:
        ...
        print("[JARVIS] ❌ Failed to initialize the UI window.")
        traceback.print_exc()
        return
